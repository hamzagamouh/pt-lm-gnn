...
# Data folder where you have the processed datasets (in zip format)
...
for LIGAND in tqdm(LIGANDS):
    ...
    gcn_model.load_state_dict(torch.load(f"/home/gamouhh/files/models/{EMB_NAME}_{LIGAND}_th_{CUTOFF}_{MODEL_CLASS}_shallow_fold_model.pt"))
    gcn_model.eval()

    MODEL_CLASS="gat"
    gat_model = GNN(MODEL_CLASS,LAYERS[MODEL_CLASS]).cuda()
    gat_model.load_state_dict(torch.load(f"/home/gamouhh/files/models/{EMB_NAME}_{LIGAND}_th_{CUTOFF}_{MODEL_CLASS}_shallow_fold_model.pt"))
    gat_model.eval()
